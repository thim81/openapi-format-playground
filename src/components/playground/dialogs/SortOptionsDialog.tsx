...
const SortOptionsDialog: React.FC<SortOptionsDialogProps> = ({
  ...
  const [importUrlValue, setImportUrlValue] = useState(
    'https://github.com/thim81/openapi-format/blob/main/defaultSort.json',
  );
  ...
  const loadContent = async (content: string) => {
    try {
      ...
      toast({
        title: 'Invalid file',
        description: 'Could not parse as a valid sort configuration.',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleLoadFromUrl = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: err?.message || 'Could not fetch the URL.',
        variant: 'destructive',
      });
    } finally {
    ...
    if (!isAdvanced) {
      ...
      if (Object.keys(parsed).length === 0 && rawValue.trim().length > 0) {
        toast({
          ...
          description: 'Could not parse sort configuration.',
          variant: 'destructive',
        });
        return;
      }
