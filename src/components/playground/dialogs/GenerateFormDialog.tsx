...
const placeholderOptions = [
  '<operationId>', '<method>', '<path>', '<pathRef>',
  '<tag>', '<tag1>', '<tag2>', '<tagn>',
  '<pathPart1>', '<pathPart2>', '<pathPartn>',
];
...
const GenerateFormDialog: React.FC<GenerateFormDialogProps> = ({
  isOpen, onClose, onSubmit, openapi, generateOptions,
}) => {
  ...
  useEffect(() => {
    const generatePreview = async () => {
      try {
        const ids = await generateOperationIdPreview(openapi, operationIdTemplate, overwriteExisting);
        setPreview(ids);
      } catch {
        setPreview([]);
      }
      ...
      <DialogContent className="max-w-2xl flex flex-col gap-0 p-0 overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-wider">OperationId Template</Label>
            <Input
          ...
          <div className="flex items-center justify-between">
            <Label htmlFor="overwrite-existing" className="text-xs font-bold uppercase tracking-wider">
          ...
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-wider flex items-center gap-2">
              ...
              Preview
              {preview.length > 0 && (
                <Badge variant="secondary" className="text-[10px] font-mono">{preview.length} operations</Badge>
              )}
            </Label>
        ...
        <div className="px-6 py-4 border-t bg-muted/20 flex justify-end gap-2">
          ...
          <Button
            ...
            onClick={() => { onSubmit({ operationIdTemplate, overwriteExisting }); onClose(); }}
          >
