...
import {
  Trash2, Plus, ChevronDown, ChevronRight, ArrowUp, ArrowDown, Copy,
  Upload, Link, Download, Code2, LayoutList, Crosshair, Layers,
} from 'lucide-react';
...
const getEnabledActions = (actions: OverlayAction[]): Set<number> => (
  new Set(
    actions
      .map((action, i) => (action.enabled === false ? -1 : i))
      .filter((i) => i >= 0)
  )
);
...
const resolveJsonPath = (obj: any, path: string): { value: any; matches: number } => {
  try {
    ...
    const values = resolveJsonPathValue(obj as Record<string, unknown>, normalizedPath) as unknown[];
...
const OverlayDialog: React.FC<OverlayDialogProps> = ({
  isOpen, onClose, overlaySet, openapi, onSubmit, format,
}) => {
  ...
  const syncUiFromCode = async (): Promise<boolean> => {
    try {
      ...
      toast({ title: 'Invalid overlay code', description: 'Please fix the overlay YAML/JSON before switching to UI mode.', variant: 'destructive' });
      return false;
    }
  };

  const addAction = () => {
    const newActions = [...(overlay.actions || []), { target: '$.paths', update: {}, enabled: true }];
  ...
  const removeAction = (index: number) => {
    ...
    actionUpdateValues.forEach((v, i) => {
      if (i < index) newVals.set(i, v); else if (i > index) newVals.set(i - 1, v);
    });
  ...
  const duplicateAction = (index: number) => {
    ...
    setExpandedActions((prev) => new Set([...prev, newIndex]));
    setEnabledActions((prev) => { const s = new Set(prev); if (prev.has(index)) s.add(newIndex); return s; });
  ...
  const moveAction = (index: number, direction: 'up' | 'down') => {
    ...
    const swapInSet = (set: Set<number>) => {
      ...
      const hadOld = next.has(index); const hadNew = next.has(newIndex);
      if (hadOld) next.add(newIndex); else next.delete(newIndex);
      if (hadNew) next.add(index); else next.delete(index);
      return next;
    };
    ...
    setActionUpdateValues((prev) => {
      ...
      const a = prev.get(index); const b = prev.get(newIndex);
      if (a !== undefined) next.set(newIndex, a); else next.delete(newIndex);
      if (b !== undefined) next.set(index, b); else next.delete(index);
      return next;
    });
  };
  ...
  const handleSubmit = async () => {
    if (mode === 'code') {
      try {
        ...
        toast({ title: 'Invalid overlay code', description: 'Fix YAML/JSON errors before applying.', variant: 'destructive' });
        return;
      }
    } else {
  ...
  const handleDownload = async () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `overlay.${format}`; a.click();
  ...
  const handleImportUrl = async () => {
    ...
    try {
      ...
      if (mode === 'code') { setCodeValue(text); }
      else {
        ...
        <div className="px-6 pt-5 pb-4 border-b bg-gradient-to-b from-muted/50 to-transparent">
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <div className="flex items-center gap-1.5">
              ...
              >
                {mode === 'ui' ? <Code2 className="h-3.5 w-3.5" /> : <LayoutList className="h-3.5 w-3.5" />}
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-[1fr_120px_1fr] gap-3">
                  <div className="space-y-1.5 min-w-0">
                    ...
                    <Input
                      ...
                      onChange={(e) => setOverlay({ ...overlay, info: { ...overlay.info, title: e.target.value } })}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) => setOverlay({ ...overlay, info: { ...overlay.info, version: e.target.value } })}
                      placeholder="1.0.0"
                      className="h-9"
                    />
                  </div>
                  <div className="space-y-1.5 min-w-0">
                    <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Extends</Label>
                    <Input
                ...
                {actionsCount === 0 && (
                  <div className="border-2 border-dashed rounded-xl py-12 flex flex-col items-center gap-3 text-muted-foreground">
                    ...
                    <p className="text-xs">Add an action to start modifying your OpenAPI document</p>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {matchCount > 0 && (
                              <Badge variant="default" className="text-[10px] px-1.5 py-0 h-[18px] font-mono bg-primary/15 text-primary border-0">
                                {matchCount} match
                              </Badge>
                            )}
                        ...
                        <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                            ...
                            <Checkbox
                              ...
                              onCheckedChange={(checked) => {
                                ...
                                setEnabledActions((prev) => {
                                  const next = new Set(prev);
                                  if (nextEnabled) next.add(i); else next.delete(i);
                                  return next;
                                });
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => moveAction(i, 'up')} disabled={i === 0}>
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => moveAction(i, 'down')} disabled={i === actionsCount - 1}>
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => duplicateAction(i)}>
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={() => removeAction(i)}>
                        ...
                        <div className="border-t bg-card">
                          <div className="grid grid-cols-2 divide-x min-h-[260px]">
                            ...
                            <div className="p-4 space-y-4">
                              <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                  <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Target (JSONPath)</Label>
                                  {matchCount > 0 ? (
                                    <span className="text-[11px] font-bold text-primary bg-primary/15 px-2 py-0.5 rounded-full">{matchCount} match</span>
                                  ) : (
                                    <span className="text-[11px] text-destructive font-medium">no match</span>
                                  )}
                                </div>
                                <Input
                                  value={action.target}
                                ...
                                <Button
                                  ...
                                  onClick={() => { setPickerIndex(i); setIsPickerOpen(true); }}
                                >
                              ...
                              <div className="space-y-2">
                                ...
                                <Select value={actionKind} onValueChange={(v) => handleActionTypeChange(i, v)}>
                            ...
                            <div className="p-4 bg-muted/40 flex flex-col">
                              <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Target Preview</Label>
                              <div className="flex-1 border-2 rounded-lg bg-background p-3 overflow-auto font-mono min-h-[200px]">
                                {preview ? (
                                  ...
                                  <div className="h-full flex items-center justify-center">
                                    <p className="text-xs text-muted-foreground italic">No match — adjust the JSONPath target</p>
                                  </div>
                                )}
                              </div>
                            </div>
                ...
                {actionsCount > 0 && (
                  <div className="flex justify-end pt-1">
                    <Button variant="outline" size="sm" className="h-8 text-xs gap-1.5" onClick={addAction}>
                      <Plus className="h-3.5 w-3.5" /> Add Action
                    </Button>
                  </div>
                )}
