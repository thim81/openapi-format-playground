...
const InstructionsDialog: React.FC<InstructionsDialogProps> = ({
  isOpen, onClose, format, sortSet, filterSet, casingSet, generateSet,
  overlaySet, sort, keepComments, toggleFilter, toggleGenerate, toggleCasing, toggleOverlay,
}) => {
  ...
  const filterFileDocker = filterContent?.length ? ` --filterFile /workspace/${filterFileName}` : '';
  ...
  const overlayFileOption = overlaySet?.length && toggleOverlay ? ` --overlayFile ${overlayFileName}` : '';
  const overlayFileDocker = overlaySet?.length && toggleOverlay ? ` --overlayFile /workspace/${overlayFileName}` : '';
  ...
  useEffect(() => {
    const buildConfig = async () => {
      ...
      if (typeof filterContent === 'string' && filterContent.length) {
        try { filterOps = await parseString(filterContent); } catch {}
      }
      if (typeof sortSet === 'string' && sortSet.length) {
        try { sortOps = await parseString(sortSet); } catch {}
      }
      if (typeof generateSet === 'string' && generateSet.length) {
        try { generateOps = await parseString(generateSet); } catch {}
      }
      if (typeof casingSet === 'string' && casingSet.length) {
        try { casingOps = await parseString(casingSet); } catch {}
      }
      if (typeof overlaySet === 'string' && overlaySet.length) {
        try { overlayOps = await parseString(overlaySet); } catch {}
      }
    ...
    void buildConfig();
  }, [sort, keepComments, filterContent, sortSet, generateSet, casingSet, overlaySet, format, toggleGenerate, toggleCasing, toggleOverlay, fileExt]);
  ...
  const hasAnyOptionSet = useMemo(
    () => [sortContent, filterContent, generateSet || '', casingSet || '', overlaySet || ''].some((s) => s.length > 0),
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[92vw] max-w-[92vw] h-[88vh] max-h-[88vh] overflow-hidden flex flex-col">
        ...
        <div className="text-sm text-muted-foreground space-y-1 mb-2">
          ...
          <p>
            The online playground provides an initial set of options of OpenAPI-Format, more options can be found in the{' '}
            <a
          ...
          <div className="flex-1 min-h-0 overflow-auto space-y-4 pr-1">
            <TabsContent value="npx" className="space-y-2 m-0">
              ...
              <div className="relative">
                <pre className="bg-muted p-3 rounded-md text-xs font-mono overflow-x-auto whitespace-pre-wrap">{npxCommand}</pre>
                <Button variant="ghost" size="icon" className="absolute top-1 right-1 h-6 w-6" onClick={() => copyToClipboard(npxCommand)}>
                  <Copy className="h-3 w-3" />
                </Button>
              </div>
              <ol className="list-decimal list-inside ml-2 text-sm" start={3}>
                <li>Review the command and ensure that the OpenAPI input and output match your local or remote file.</li>
              </ol>
            </TabsContent>
            ...
            <TabsContent value="npm" className="space-y-2 m-0">
              ...
              <pre className="bg-muted p-3 rounded-md text-xs font-mono">npm install -g openapi-format</pre>
              ...
              <div className="relative">
                <pre className="bg-muted p-3 rounded-md text-xs font-mono overflow-x-auto whitespace-pre-wrap">{npmCommand}</pre>
                <Button variant="ghost" size="icon" className="absolute top-1 right-1 h-6 w-6" onClick={() => copyToClipboard(npmCommand)}>
                  <Copy className="h-3 w-3" />
                </Button>
              </div>
              <ol className="list-decimal list-inside ml-2 text-sm" start={3}>
            ...
            <TabsContent value="docker" className="space-y-2 m-0">
              ...
              <pre className="bg-muted p-3 rounded-md text-xs font-mono">docker pull ghcr.io/thim81/openapi-format:latest</pre>
              <ol className="list-decimal list-inside ml-2 space-y-2 text-sm" start={2}>
                <li>Run the Docker container with the appropriate options:</li>
              </ol>
              <div className="relative">
                <pre className="bg-muted p-3 rounded-md text-xs font-mono overflow-x-auto whitespace-pre-wrap">{dockerCommand}</pre>
                <Button variant="ghost" size="icon" className="absolute top-1 right-1 h-6 w-6" onClick={() => copyToClipboard(dockerCommand)}>
                  <Copy className="h-3 w-3" />
                </Button>
              </div>
              <ol className="list-decimal list-inside ml-2 text-sm" start={3}>
            ...
            <TabsContent value="config" className="space-y-3 m-0">
              ...
              <div className="h-[45vh] border rounded-md overflow-hidden">
                <MonacoEditor value={configFileContent} language={format} readOnly showLineNumbers={false} />
              </div>
              <ol className="list-decimal list-inside ml-2 space-y-2 text-sm" start={2}>
              ...
              <div className="relative">
                ...
                <Button
                  ...
                  onClick={() => copyToClipboard(`npx openapi-format openapi.${fileExt} --configFile oaf-config.${fileExt}`)}
                >
                  <Copy className="h-3 w-3" />
                </Button>
              </div>
              <ol className="list-decimal list-inside ml-2 text-sm" start={3}>
                <li>Review the options and ensure that the OpenAPI input and output match your local or remote file.</li>
              </ol>
            </TabsContent>
              ...
              <div className="space-y-3 pt-2 border-t">
                ...
                {sortContent.length > 0 && sort && (
                  <div className="space-y-2">
                    ...
                    <div className="h-[14vh] border rounded-md overflow-hidden">
                      <MonacoEditor value={sortContent} language={format} readOnly showLineNumbers={false} />
                    </div>
                  </div>
                )}
                ...
                {filterContent.length > 0 && (
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <DownloadButton content={filterContent} filename="oaf-filter" format={format} />
                    </div>
                    <div className="h-[14vh] border rounded-md overflow-hidden">
                      <MonacoEditor value={filterContent} language={format} readOnly showLineNumbers={false} />
                    </div>
                  </div>
                )}
                ...
                {generateSet && generateSet.length > 0 && toggleGenerate && (
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <DownloadButton content={generateSet} filename="oaf-generate" format={format} />
                    </div>
                    <div className="h-[14vh] border rounded-md overflow-hidden">
                      <MonacoEditor value={generateSet} language={format} readOnly showLineNumbers={false} />
                    </div>
                  </div>
                )}
                ...
                {casingSet && casingSet.length > 0 && toggleCasing && (
                  <div className="space-y-2">
                    ...
                    <div className="h-[14vh] border rounded-md overflow-hidden">
                      <MonacoEditor value={casingSet} language={format} readOnly showLineNumbers={false} />
                    </div>
                  </div>
                )}
                ...
                {overlaySet && overlaySet.length > 0 && toggleOverlay && (
                  <div className="space-y-2">
                    ...
                    <div className="h-[14vh] border rounded-md overflow-hidden">
                      <MonacoEditor value={overlaySet} language={format} readOnly showLineNumbers={false} />
                    </div>
                  </div>
                )}
              </div>
